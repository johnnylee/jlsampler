...
//
// Of course it's always possible that I had an error in the code that was
// causing issues.
func InterpLinear16(y1, y2, mu float64) int16 {
	return int16(Clamp16(y1*(1-mu) + y2*mu))
}
...
func (s *Sample) Stretched(semitones float64) *Sample {
	...
	sNew.UpdateCropThresh()
	sNew.UpdateRms()

	return sNew
}

func (s *Sample) UpdateCropThresh() {
	for s.Idx0 = 0; s.Idx0 < s.Len; s.Idx0++ {
		if absMaxSampleVal(s, s.Idx0) >= controls.CropThresh {
			return
		}
	}
	s.UpdateRms()
...
func (s *Sample) UpdateRms() {
	rms := 0.0
	num := 0.0
	var x float64
		
	imin := s.Idx0
	imax := s.Idx0 + int(sampleRate * controls.RmsTime)
	if imax > s.Len {
		imax = s.Len
	}
